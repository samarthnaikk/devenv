...
class OllamaBackendTest(unittest.TestCase):
    ...
    def test_chat_parses_relaxed_tool_call_json_response(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            core = OllamaAICore(workspace_path=tempdir, model="qwen2.5:3b")
            core.register_tool(FakeTool())
            stream = json.dumps(
                {
                    "message": {
                        "content": '{"type":"tool_call","tool_name:":"read_file","arguments":{"path":"README.md"}}'
                    },
                    "done": True,
                }
            )
    ...
    def test_chat_uses_blueprint_schema_and_zero_temperature_for_planner_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            core = OllamaAICore(workspace_path=tempdir, model="qwen2.5:3b")
            core.register_tool(FakeTool())
            captured = {}
            ...
            with patch.object(core, "_stream_chat", side_effect=fake_stream):
                response = core.chat(
                    messages=[
                        {"role": "system", "content": "PLANNER_OUTPUT_MODE: blueprint_json"},
                        {"role": "user", "content": "Plan the work"},
    ...
    def test_compile_messages_includes_tool_schema_payload(self) -> None:
        ...
        self.assertIn("Available tools:", compiled[0]["content"])
