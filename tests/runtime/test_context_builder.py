...
class ContextBuilderServiceTest(unittest.TestCase):
    ...
    def test_retrieve_sessions_returns_structured_hits_and_chunks(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            workspace = Path(tempdir) / "workspace"
            workspace.mkdir()

            codex_root = Path(tempdir) / ".codex"
            sessions_dir = codex_root / "sessions" / "2026" / "09" / "10"
            sessions_dir.mkdir(parents=True)
            session_id = "session-retrieval"
            (codex_root / "session_index.jsonl").write_text(
                json.dumps({"id": session_id, "thread_name": "Retrieval engine work", "updated_at": "2026-09-10T12:00:00Z"}) + "\n",
                encoding="utf-8",
            )
            (sessions_dir / f"rollout-2026-09-10T12-00-00-{session_id}.jsonl").write_text(
                "\n".join(
                    [
                        json.dumps({"timestamp": "2026-09-10T11:59:00Z", "type": "session_meta", "payload": {"id": session_id, "cwd": str(workspace)}}),
                        json.dumps(
                            {
                                "timestamp": "2026-09-10T11:59:01Z",
                                "type": "event_msg",
                                "payload": {
                                    "type": "agent_message",
                                    "message": "The retrieval engine fuses lexical and semantic recall before selecting chunks.",
                                },
                            }
                        ),
                    ]
            ...
            for _ in range(60):
                if service.indexing_status()["completed"]:
                    break
                time.sleep(0.05)
            result = service.retrieve_sessions("how does the retrieval engine fuse recall?")

        self.assertEqual(result.query, "how does the retrieval engine fuse recall?")
        self.assertGreaterEqual(len(result.hits), 1)
        hit = result.hits[0]
        self.assertEqual(hit.session_id, session_id)
        self.assertTrue(hit.chunks)
        self.assertTrue(
            any("fuses lexical and semantic recall" in chunk.text for chunk in hit.chunks)
        )
        self.assertIn("Retrieved Session Context", result.context)
        self.assertIn(session_id, result.session_ids)
        self.assertTrue(result.metadata["index_ready"])

    def test_retrieve_sessions_reports_no_access_without_grants(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            workspace = Path(tempdir) / "workspace"
            workspace.mkdir()
            service = ContextBuilderService(
                str(workspace),
                provider_configs=(
                    ExternalSessionProviderConfig(provider="codex", root_path=str(Path(tempdir) / ".codex"), index_path="session_index.jsonl"),
                ),
            )
            service.set_runtime_allowed_providers(set())
            result = service.retrieve_sessions("anything at all")

        self.assertEqual(result.hits, ())
        self.assertIn("access has not been granted", result.metadata["context_match_reason"])

    def test_retrieve_sessions_returns_empty_for_blank_query(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            workspace = Path(tempdir) / "workspace"
            workspace.mkdir()
            service = ContextBuilderService(
            ...
            result = service.retrieve_sessions("   ")
