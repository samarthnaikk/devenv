...
@dataclass(frozen=True)
class SessionRetrievalChunk:
    """A single retrieved chunk, ready to be presented in the retrieval TUI."""
...
class SessionRetrievalHit:
    """A ranked prior session with the chunks that matched the query."""
    ...
    chunks: tuple[SessionRetrievalChunk, ...] = ()
...
class SessionRetrievalResult:
    """Structured output of a retrieval-only request."""

    query: str
    context: str = ""
    hits: tuple[SessionRetrievalHit, ...] = ()
    metadata: dict[str, Any] = field(default_factory=dict)

    @property
    def session_ids(self) -> tuple[str, ...]:
        return tuple(hit.session_id for hit in self.hits)
...
class ContextBuilderService:
    ...
    def retrieve_sessions(
        ...
        *,
        ...
        """Retrieve prior sessions and their matching chunks without invoking a model.

        This is the read-only entry point used by the retrieval TUI mode. It reuses the
        exact ranking pipeline the runtime uses, but returns structured hits instead of a
        prompt-ready context block so callers can present what was actually recalled.
        """
        ...
        if not query:
            return SessionRetrievalResult(
                ...
                metadata={
                    "context_match_state": "new_context",
                    "context_match_reason": "No query was provided.",
                },
            )
        if self.runtime_allowed_providers == set():
            return SessionRetrievalResult(
                ...
                metadata={
                    "context_match_state": "new_context",
                    "context_match_reason": "External session access has not been granted.",
                },
            )

        candidate_providers = self._resolve_retrieval_providers(provider_names)
        ...
        for provider_name in candidate_providers:
            try:
                matches, provider, metadata = self._select_runtime_matches_for_provider(
                    query, provider_name=provider_name
                )
            except Exception as exc:
                logger.warning("Retrieval failed for provider=%s: error=%s", provider_name, exc)
                continue
            ...
            for match in matches:
                ...
                if not isinstance(summary, ExternalSessionSummary):
                    continue
                key = (summary.provider, summary.session_id)
                if key in seen_sessions:
                    continue
                seen_sessions.add(key)
                hits.append(
                    self._retrieval_hit_for_match(query, match, provider, max_chunks=max_chunks)
                )

        hits.sort(
            key=lambda hit: (hit.strong_match, hit.score, hit.semantic_score, hit.updated_at),
            reverse=True,
        )
        limited_hits = tuple(hits[: max(max_sessions, 0)])
        metadata: dict[str, Any] = {
            "index_ready": index_ready,
            "matched_providers": matched_providers,
            "candidate_sessions": len(hits),
            "returned_sessions": len(limited_hits),
        }
        if limited_hits:
            metadata["context_match_state"] = "reused_prior_context"
            metadata["context_match_reason"] = (
                f"Matched {len(limited_hits)} prior session(s) across "
                f"{len(matched_providers)} provider(s)."
            )
        else:
            metadata["context_match_state"] = "new_context"
            metadata["context_match_reason"] = "No strong prior-session match was found."
        return SessionRetrievalResult(
            query=query,
            context=_render_retrieval_context(limited_hits),
            hits=limited_hits,
            metadata=metadata,
        )
    ...
    def _resolve_retrieval_providers(
        self,
        provider_names: list[str] | tuple[str, ...] | None,
    ) -> list[str]:
        if provider_names:
            resolved = [name for name in provider_names if name in self.providers]
            if self.runtime_allowed_providers is not None:
                resolved = [name for name in resolved if name in self.runtime_allowed_providers]
            return resolved
        return self._candidate_provider_names()

    def _retrieval_hit_for_match(
        self,
        query: str,
        match: dict[str, Any],
        ...
        *,
        ...
        ranked_chunks = self._ranked_chunks_for_match(query, match, provider, limit=max_chunks)
        chunks = tuple(
            SessionRetrievalChunk(
        ...
        return SessionRetrievalHit(
            provider=summary.provider,
            session_id=summary.session_id,
            title=summary.title,
            workspace_path=summary.workspace_path,
            updated_at=summary.updated_at,
            score=float(match.get("score", 0) or 0.0),
            semantic_score=float(match.get("semantic_score", 0.0) or 0.0),
            strong_match=bool(match.get("strong_match", False)),
            chunks=chunks,
        )

    def _ranked_chunks_for_match(
        self,
        query: str,
        match: dict[str, Any],
        provider: ExternalSessionProvider,
        *,
        limit: int,
    ) -> list[tuple[ExternalSessionChunk, float]]:
        candidates = [
            chunk for chunk in (match.get("chunks") or []) if isinstance(chunk, ExternalSessionChunk)
        ]
        if not candidates:
            summary = match.get("summary")
            ...
            if session_id:
                try:
                    candidates = provider.build_index_chunks(session_id)
                except Exception as exc:
                    logger.debug("Failed to build retrieval chunks for session=%s: %s", session_id, exc)
                    candidates = []
        if not candidates:
            detail = match.get("detail")
            if isinstance(detail, ExternalSessionDetail) and detail.messages:
                candidates = _build_chunks_from_messages(
                    detail.summary, list(detail.messages), source="detail"
                )
        return _rank_session_chunks(query, list(candidates), limit=limit)
...
def _rank_session_chunks(
    ...
    *,
    ...
    if not chunks or limit <= 0:
        return []
    tokens = _tokenize(task)
    scored: list[tuple[float, ExternalSessionChunk]] = []
    for chunk in chunks:
        haystack = chunk.search_text
        exact_hits = _exact_prompt_hits(tokens, [haystack])
        token_hits = sum(1 for token in tokens if _token_matches(token, haystack))
        score = float((exact_hits * 4) + (token_hits * 2))
        if chunk.role in {"user", "assistant"}:
            score += 1.0
        if chunk.source == "reasoning":
            score += 0.5
    ...
    return [(chunk, score) for score, chunk in (positive or scored)[:limit]]


def _render_retrieval_context(hits: tuple[SessionRetrievalHit, ...]) -> str:
    if not hits:
        return ""
    lines: list[str] = ["## Retrieved Session Context"]
    for hit in hits:
        lines.append("")
        lines.append(f"### {hit.title} ({hit.provider})")
        workspace = hit.workspace_path or "unknown workspace"
        lines.append(f"- session `{hit.session_id}` · workspace {workspace} · score {hit.score:.1f}")
        for chunk in hit.chunks:
            prefix = (
                "User asked"
                if chunk.role == "user"
                else "Assistant reported"
                if chunk.role == "assistant"
                else "Tool output"
            )
            lines.append(f"- {prefix}: {_normalize_whitespace(chunk.text)}")
    return "\n".join(lines).strip()
